...
// jaja, jaja,kaka, oaskdas, asdas,dfasdas
    // jaja,jaja,jaja,jaja
    public ArrayList<String> getAllMealNames() {
    ...
    @Override
    public ArrayList<String> getMealNames(String search) {
        ...
        try {
            for (String s : searchArray) {
                ...
                ResultSet rs = st.executeQuery("SELECT meal_name FROM meal WHERE " +
                        "lower (meal_name) like '%" + s.toLowerCase() + "%' " +
                        " OR lower (meal_desc) like '%" + s.toLowerCase() + "%'");
                while (rs.next()) {
                    mealNameList.add(rs.getString(1));
                }
            }
